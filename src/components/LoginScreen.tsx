  ...
  return (
    <label className="group relative block">
      <input
        ...
        onBlur={onBlur}
        className="peer h-16 w-full rounded-2xl border border-line bg-panel/80 px-4 pb-2 pt-6 text-[15px] font-semibold outline-none transition focus:border-ink focus:ring-4 focus:ring-ink/5"
      />
...
export function LoginScreen() {
  ...
  return (
    <main className="grain relative grid min-h-[100dvh] place-items-center overflow-hidden px-5 py-10">
      ...
      <div className="absolute bottom-[-8rem] right-[-7rem] h-80 w-80 rounded-full bg-flare/10 blur-3xl" />
      <motion.section
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ type: "spring", stiffness: 220, damping: 25 }}
        className="relative w-full max-w-md rounded-[2rem] border border-line bg-panel/75 p-6 shadow-card backdrop-blur-2xl sm:p-8"
      >

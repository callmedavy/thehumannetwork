...
export function ActionRail({ onAction, onRead, canVote, canMarkRead }: { onAction: (direction: SwipeDirection) => void; onRead: () => void; canVote: boolean; canMarkRead: boolean }) {
  return (
    <div className="flex items-center justify-center gap-3" aria-label="Post actions">
      ...
      <button type="button" onClick={onRead} disabled={!canMarkRead} aria-label="Mark post as read and show next post" className="h-12 rounded-full border border-line bg-panel px-5 text-xs font-extrabold text-muted shadow-soft transition hover:bg-ink hover:text-panel active:scale-95 disabled:cursor-not-allowed disabled:opacity-30">
        Mark Read
      </button>

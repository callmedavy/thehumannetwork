...
The Human Network is a mobile-first, swipe-based Lemmy client. It turns a federated feed into a focused stack of tactile cards: swipe right to upvote, left to downvote, or tap through to the full post and threaded conversation.
...
- Infinite post queue with scope filtering and a default view of today’s most-upvoted posts
...
- Four responsive color themes with reduced-motion support and loading skeletons
...
The Human Network intentionally uses `localStorage` rather than a server database. Credentials and personal preferences remain on the user’s device, while posts, votes, and comments go directly between the browser and the selected Lemmy instance.
...
The client sends authenticated requests using both the bearer header and the legacy `auth` field/query convention for broad `/api/v3` compatibility. Most Lemmy instances permit browser requests. If an instance blocks cross-origin traffic, The Human Network displays a friendly error recommending another instance or a small Netlify proxy function.
...
- The Human Network does not require an application account or server-side user database.
